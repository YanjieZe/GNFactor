...
class CustomRLBenchEnv(RLBenchEnv):
    ...
    def set_variation(self, variation_number):
        variation_number = variation_number % self._task.variation_count()
...
class CustomMultiTaskRLBenchEnv(MultiTaskRLBenchEnv):
    ...
    def step(self, act_result: ActResult) -> Transition:
        ...
        try:
            obs, reward, terminal = self._task.step(action) # main part of the runnning time
            if reward >= 1:
                success = True
                reward *= self._reward_scale
            else:
    ...
    def reset_to_demo(self, i, variation_number=-1, novel_command=None):
