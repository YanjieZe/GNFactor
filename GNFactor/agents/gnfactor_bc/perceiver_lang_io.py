...
class PerceiverVoxelLangEncoder(nn.Module):
    def __init__(
        ...
        # # feature fusion for nerf
        # self.conv_nerf_feat = Conv3DBlock(
        #     self.im_channels if (self.no_perceiver or self.no_skip_connection) else self.im_channels * 2,
        #     self.im_channels*2,
        #     kernel_sizes=3,
        #     strides=1, norm=None, activation=activation)
    ...
    def forward(
        ...
        SAVE_INTERNAL_TENSOR = False
        if SAVE_INTERNAL_TENSOR:
            counter = self.counter()
            import os
            os.makedirs('/home/yanjieze/projects/nerf-act/internal_feat_pkl', exist_ok=True)
            self.save_tensor(ins, f'/home/yanjieze/projects/nerf-act/internal_feat_pkl/input_voxel_{counter}.pkl')
            print("save input voxel tensor.")
            self.save_tensor(d0, f'/home/yanjieze/projects/nerf-act/internal_feat_pkl/deep_voxel_{counter}.pkl')
            print("save deep voxel tensor.")
            self.save_tensor(lang_token_embs, f'/home/yanjieze/projects/nerf-act/internal_feat_pkl/lang_token_embs_{counter}.pkl')
            print("save lang_token_embs tensor.")
        ...
        if SAVE_INTERNAL_TENSOR:
            print("save reduced lang token")
            self.save_tensor(l, f'/home/yanjieze/projects/nerf-act/internal_feat_pkl/reduced_lang_token_{counter}.pkl')
        ...
        for it in range(self.iterations):
            ...
            if SAVE_INTERNAL_TENSOR:
                attention_matrix_1st = cross_attn.get_attention_matrix(x, context=ins, mask=mask)
                self.save_tensor(attention_matrix_1st, f'/home/yanjieze/projects/nerf-act/internal_feat_pkl/attention_matrix_1st_{counter}.pkl')
            ...
            for self_attn, self_ff in self.layers:
                ...
                layer_counter += 1
                if SAVE_INTERNAL_TENSOR :
                    attention_matrix_last = self_attn.get_attention_matrix(x)
                    self.save_tensor(attention_matrix_last, f'/home/yanjieze/projects/nerf-act/internal_feat_pkl/attention_matrix_{layer_counter}th_{counter}.pkl')
                x = self_ff(x) + x
        ...
        if SAVE_INTERNAL_TENSOR :
            attention_matrix_last = self.decoder_cross_attn.get_attention_matrix(ins, context=x)
            self.save_tensor(attention_matrix_last, f'/home/yanjieze/projects/nerf-act/internal_feat_pkl/attention_matrix_last_{counter}.pkl')
        # latents: [B,8077,128]
        ...
        # upsample
        u0 = self.up0(latents) # [B,128,20,20,20] -> [B,128,100,100,100]
        
        if SAVE_INTERNAL_TENSOR:
            self.save_tensor(u0, f'/home/yanjieze/projects/nerf-act/internal_feat_pkl/upsampled_feat_voxel_{counter}.pkl')
        ...
        if self.no_skip_connection:
            ...
            u = self.final(torch.cat([d0, u0], dim=1)) # 128x100^3
        ...
        # translation decoder
        trans = self.trans_decoder(u) # 1x100^3
        ...
        return trans, rot_and_grip_out, collision_out, voxel_feat, multi_scale_voxel_list, l
