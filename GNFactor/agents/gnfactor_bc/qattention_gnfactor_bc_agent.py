...
class QAttentionPerActBCAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        LOAD_MODEL_FOR_DEBUG = False
        if LOAD_MODEL_FOR_DEBUG:
            weight="/home/yanjieze/projects/nerf-act/peract/logs/NERFODISEACTv4_BC_MT10D20_improvediffv2/seed0/weights/100000/QAttentionAgent_layer0.pt"
            # weight = "/home/yanjieze/projects/nerf-act/peract/logs/NERFODISEACTv4_BC_D10S5_shapesort/seed0/weights/50000/QAttentionAgent_layer0.pt"
            ...
            print("!!!!!!!!")
            print("!!!!!!!!")
            print(f"we are loading a pretrained model for debug!")
            print("!!!!!!!!")
            print("!!!!!!!!")
            print("!!!!!!!!")
            print("lang_goal:")
            print(replay_sample['lang_goal'])
        ...
        if VIS_WITH_GRAD_CAM:
            ...
            score.backward()
            # 用梯度进行加权，计算 Grad-CAM
            grad_cam_map = torch.zeros(self.feature.shape[2:], dtype=torch.float).cuda()
            for i in range(self.gradient.shape[1]):
                grad_cam_map += self.feature[0,i,...] * self.gradient[0,i,...]

            # 使用 ReLU 移除负值
            grad_cam_map = F.relu(grad_cam_map)
            from helpers import utils
            utils.save_tensor(grad_cam_map, '/home/yanjieze/projects/nerf-act/internal_feat_pkl/grad_cam_map.pkl')
